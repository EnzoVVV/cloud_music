        ...
        <transition name='mini-list'>
            <div class='mini-list'>
                ...
                <div class='header' @touchstart='headerTouchStart' @touchmove='headerTouchMove' @touchend='headerTouchEnd' ref='header'>
                    ...
                    <IconSvg icon-class='back' class='clear'></IconSvg>
                </div>
                   ...
                   <scroll class='scroll' :listen-scroll='listenScroll' :probe-type='probeType' @scroll='handleScroll' ref='scroll'>
                       <transition-group name='list' tag='ul'>
                           <li v-for='item in mockList' :key='item.id' @click='select(item)' ref='item' class='item'>
                               ...
                               <div class='delete' @click.stop='delteOne(item)'><IconSvg icon-class='back'></IconSvg></div>
    ...
    export default {
        ...
        computed: {
            modeText() {
                ...
                const songsCount = this.secquenceList.length
            ...
            headerTouchEnd(e) {
                ...
                if(this.headerTouch.totalDiff === undefined) {
                    ...
                    if(el.className == 'modeIcon' || el.className == 'modeText') {
                        this.toggleMode()
                    } else if(el.classList && el.classList.contains('clear')) {
                        // IconSvg有多个className，这里在classList中用classList的contains方法检测
                        this.confirm()
                    }
                    this.headerTouch = {}
                    return   
        ...
        mounted() {
            ...
            this.maxMoveDistance = this.$refs.header.getBoundingClientRect().bottom
        }
    }
</script>
