<template>
    <div class='wrapper' :style='computedStyle'>
        <div class='marquee' :style='computedStyle' @onMouseover='curTime=pause' @onMouseout='curTime=time'>
...
<script>
    export default {
        ...
        computed: {
            computedStyle() {
                return {
                    width: this.width ? `${thsi.width}px` : '100%',
                    height: this.height ? `${thsi.height}px` : '44px'
                }
            }
        },
        watch: {
        ...
        methods: {
            ...
            scrollmarquee() {
                // TODO，如果这里不取整, tail刚开始时就是按向上取整的speed移动的,why?
                this.speed = parseInt(this.distance/this.curTime * this.fps)
                this.scroll(this.front, this.tail)
                this.scroll(this.tail, this.front)
            },
            scroll(el1, el2) {
                if(parseInt(el1.style.left) < -this.actualWidth) {
                    // 到头了，移动到最后面
                    el1.style.left = parseInt(el2.style.left) + this.distance + 'px'
                } else {
                    // 向前移动
                    el1.style.left = parseInt(el1.style.left) - this.speed + 'px'
                }
            }
        },
        created() {
